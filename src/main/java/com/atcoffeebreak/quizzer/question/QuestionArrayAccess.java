...
public class QuestionArrayAccess implements QuestionAccessible{
    private List<Question> questionList;
    static private final int MAX_NUMBER_OF_QUESTIONS_PER_PAGE = 10;

    /**
     * Constructor. Loads the array with  questions.
     */
    public QuestionArrayAccess() {
        this.loadQuestionArray();
    }

    @Override
    public Question getQuestionByIndex(long index) {
    ...
    @Override
    public List<Question> getQuestionList(long offset) {
        long start = offset;
        if (start < 0) {
            start = 0;
        }
    ...
    @Override
    public List<Question> getSpecifiedQuestionList(long... id) {
    ...
    /*
        Creates  questions and adds them to an array.
    */

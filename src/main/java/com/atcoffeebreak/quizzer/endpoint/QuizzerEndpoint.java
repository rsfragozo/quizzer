package com.atcoffeebreak.quizzer.endpoint;

import com.atcoffeebreak.quizzer.question.Question;
import com.atcoffeebreak.quizzer.question.QuestionAccessible;
import com.atcoffeebreak.quizzer.question.QuestionArrayAccess;
...
/**
 * REST endpoint for questions. This supports random  questions, a list of
 * 10 questions, a random question, and a question count.
 *
 */
@Path("/questions")
public final class QuizzerEndpoint {

    private final Date questionsUpdatedDate = new Date();
    private final QuestionAccessible dataAccess = new QuestionArrayAccess();
    ...
    public Response getQuestion(@Context UriInfo uri,
        ...
        if (idString.trim().equalsIgnoreCase("random")) {
            ...
            response = Response.ok(question)
                    .lastModified(question.getLastUpdate())
                    .location(uri.getRequestUri())
                    .build();
        } else {
            try {
